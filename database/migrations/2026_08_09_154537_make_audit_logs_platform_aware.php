...
return new class extends Migration
{
     ...
     * plutôt que via Blueprint::change() — MySQL uniquement : SQLite (utilisé
     * par la suite de tests, voir phpunit.xml) n'a pas d'équivalent à
     * `MODIFY` et laisse de toute façon la colonne peu contraignante sur les
     * NULL en pratique, donc l'ignorer ici n'affaiblit pas les tests.
     */
    public function up(): void
    {
        if (DB::getDriverName() !== 'sqlite') {
            DB::statement('ALTER TABLE audit_logs MODIFY application_id BIGINT UNSIGNED NULL');
        }
    ...
    public function down(): void
    {
        ...
        if (DB::getDriverName() !== 'sqlite') {
            DB::statement('ALTER TABLE audit_logs MODIFY application_id BIGINT UNSIGNED NOT NULL');
        }
    }
};
